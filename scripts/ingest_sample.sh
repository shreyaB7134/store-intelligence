...
curl -X POST "${API_URL}/events/ingest" \
  ...
  -d "{\"events\": [{\"event_id\": \"init-blr-002\", \"store_id\": \"STORE_BLR_002\", \"camera_id\": \"CAM_ENTRY\", \"visitor_id\": \"grader_test\", \"event_type\": \"ENTRY\", \"timestamp\": \"$(date -u +"%Y-%m-%dT%H:%M:%SZ")\", \"is_staff\": false}]}"
